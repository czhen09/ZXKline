...
@property (nonatomic,strong) CAShapeLayer *vertivalMiddleDottedLayer;
...
- (void)addTimeLabel
{
    ...
    CGFloat height = 60;
    CGFloat leftSpace = 0;
    leftSpace = -(height-TimeViewHeight)/2+(MiddleBlankSpace+QuotaChartHeight-2+NewQuotaHeight);
...
- (void)drawLine
{
    ...
    if (self.isDrawDottedLine) {
        ...
        [dottedLine moveToPoint:CGPointMake(self.quotaChartHeight+self.middleBlankSpace+TimeViewHeight-4+NewQuotaHeight,self.frame.size.height/2)];
...
//        self.vertivalTopDottedLayer.lineDashPattern = @[@4, @2];
        ...
        if (NewQuotaHeight!=0&&!DrawJustKline) {
            //中间
            [self.vertivalMiddleDottedLayer removeFromSuperlayer];
            self.vertivalMiddleDottedLayer = nil;
            self.vertivalMiddleDottedLayer = [CAShapeLayer layer];
            UIBezierPath *dottedMiddleLine = [UIBezierPath bezierPath];
            [dottedMiddleLine moveToPoint:CGPointMake(NewQuotaHeight,self.frame.size.height/2)];
            [dottedMiddleLine addLineToPoint:CGPointMake(self.quotaChartHeight+NewQuotaHeight, self.frame.size.height/2)];
            //        self.vertivalBottomDottedLayer.lineDashPattern = @[@4, @2];
            self.vertivalMiddleDottedLayer.lineWidth = 0.2;
            self.vertivalMiddleDottedLayer.path = dottedMiddleLine.CGPath;
            self.vertivalMiddleDottedLayer.strokeColor = GrateLineColor.CGColor;
            [self.layer addSublayer:self.vertivalMiddleDottedLayer];
        }
        if (!DrawJustKline) {
            //下部分

...
- (void)drawWithPositionArr:(NSArray *)positionArr
{
    ...
    [positionArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (![obj isKindOfClass:[NSString class]]) {
            ...
            double pointX = [obj doubleValue];
            if ([self.quotaName isEqualToString:@"VOL"]&&NewQuotaHeight!=0) {
                pointX += NewQuotaHeight;
            }
            
            if (idx==invalidNumCount) {
                
                [self.beizerPath moveToPoint:CGPointMake(pointX, self.rowHeight*(self.startIndex+idx)+self.rowHeight/2)];
            }else{
                
                [self.beizerPath addLineToPoint:CGPointMake(pointX, self.rowHeight*(self.startIndex+idx)+self.rowHeight/2)];
            }

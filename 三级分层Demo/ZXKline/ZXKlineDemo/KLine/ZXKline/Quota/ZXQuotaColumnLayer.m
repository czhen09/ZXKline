...
- (instancetype)initQuotaDataArr:(NSArray *)dataArr currentDrawStartIndex:(NSInteger)startIndex rowHeight:(CGFloat)rowHeight  minValue:(double)minValue maxValue:(CGFloat)maxValue heightPerpoint:(CGFloat)heightPerpoint columnColorArr:(NSArray *)columnColorArr columnWidthType:(ColumnWidthType)columnWidthType quotaName:(NSString *)quotaName
{
    self = [super init];
    if (self) {
...
- (NSArray *)drawQuotaWithIndex:(NSInteger)index xPosition:(CGFloat)x
{
    ...
    if ([self.quotaName isEqualToString:@"VOL"]&&NewQuotaHeight!=0) {
        zeroValue += NewQuotaHeight;
    }
    ...
    }else
    {
        CGFloat startPointX = 0;
        if ([self.quotaName isEqualToString:@"VOL"]&&NewQuotaHeight!=0) {
            startPointX += NewQuotaHeight;
        }
        switch (self.columnWidthType) {
            case ColumnWidthTypeEqualLine:
                [beizerPath moveToPoint:CGPointMake(startPointX, self.rowHeight*(self.startIndex+index)+self.rowHeight/2)];
                [beizerPath addLineToPoint:CGPointMake(x+startPointX, self.rowHeight*(self.startIndex+index)+self.rowHeight/2)];
                break;
            case ColumnWidthTypeEqualCandle:
                //高度  x-QuotaBottomMargin   需要减去10
                beizerPath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(startPointX, self.rowHeight*(self.startIndex+index)+2, x,self.rowHeight-4) cornerRadius:0];
                break;
            default:
                break;
        }

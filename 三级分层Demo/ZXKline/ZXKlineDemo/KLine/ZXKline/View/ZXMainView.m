 ...
 * 装指标volLayer的数组
 */
@property (nonatomic,strong) NSMutableArray *volLayerArr;
/**
 * 装指标数据的字典数组
 */
 ...
 * 装指标数据的字典数组
 */
@property (nonatomic,strong) NSMutableArray *quotaVolDataArr;
/**
 * 装指标颜色的字典数组
 */
@property (nonatomic,strong) NSMutableArray *quotaVolColorArr;
/**
 * 装均线数据的字典数组
 */
...
- (void)drawQuotaWithType:(QuotaType)quotaType dataArr:(NSArray *)dataArr maxValue:(double)maxValue minValue:(double)minValue quotaName:(NSString *)quotaName subName:(NSString *)subName lineColor:(UIColor *)lineColor columnColorArr:(NSArray *)columnColorArr columnWidthType:(ColumnWidthType)columnWidthType
{
    ...
    if ([quotaName isEqualToString:MiddleQuotaName]) {
        ...
        [self.quotaVolDataArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([tempArr containsObject:subName]) {
            ...
            [self.quotaVolDataArr replaceObjectAtIndex:idx withObject:quotaDataDic];
            //颜色
            [self.quotaVolColorArr replaceObjectAtIndex:idx withObject:quotaColorDic];
        }else{
            [self.quotaVolDataArr addObject:quotaDataDic];
            //颜色
            [self.quotaVolColorArr addObject:quotaColorDic];
        }
    }else{
...
- (void)drawQuotaColumnwithDataArr:(NSArray *)dataArr maxValue:(double)maxValue minValue:(double)minValue quotaName:(NSString *)quotaName subName:(NSString *)subName columnColorArr:(NSArray *)columnColorArr columnWidthType:(ColumnWidthType)columnWidthType{
    ...
    if ([quotaName isEqualToString:MiddleQuotaName]&&NewQuotaHeight!=0) {
        self.quotaName = quotaName;
        self.quotaVolMaxAssert = maxValue;
        self.quotaVolMinAssert = minValue;
        self.quotaVolHeightPerPoint = NewQuotaHeight/(self.quotaVolMaxAssert - self.quotaVolMinAssert);
        ColumnWidthType type = ColumnWidthTypeEqualCandle;
        if (self.topChartType==ZXTopChartTypeTimeLine) {
            type = ColumnWidthTypeEqualLine;
        }
        
        ZXQuotaColumnLayer * MA6Layer = [[ZXQuotaColumnLayer alloc] initQuotaDataArr:dataArr currentDrawStartIndex:self.needDrawStartIndex rowHeight:self.candleWidth minValue:self.quotaVolMinAssert maxValue:self.quotaVolMaxAssert  heightPerpoint:self.quotaVolHeightPerPoint columnColorArr:columnColorArr columnWidthType:type quotaName:quotaName];
        NSMutableArray *tempArr = [NSMutableArray array];
        [self.volLayerArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        NSDictionary *quotaLayerDic = [NSDictionary dictionaryWithObjectsAndKeys:MA6Layer,subName,nil];
        if ([tempArr containsObject:subName]) {
            
            NSInteger idx = [tempArr indexOfObject:subName];
            
            NSDictionary *tempQuotaLayerDic = self.volLayerArr[idx];
            CAShapeLayer *quotaLayer = tempQuotaLayerDic[subName];
            [quotaLayer removeFromSuperlayer];
            quotaLayer = nil;
            [self.volLayerArr replaceObjectAtIndex:idx withObject:quotaLayerDic];
        }else{
            [self.volLayerArr addObject:quotaLayerDic];
        }
        [self.tableView.layer addSublayer:MA6Layer];
        return;
    }
    ...
    ZXQuotaColumnLayer * MA5Layer = [[ZXQuotaColumnLayer alloc] initQuotaDataArr:dataArr currentDrawStartIndex:self.needDrawStartIndex rowHeight:self.candleWidth minValue:self.quotaMinAssert maxValue:self.quotaMaxAssert  heightPerpoint:self.quotaHeightPerPoint columnColorArr:columnColorArr columnWidthType:type quotaName:quotaName];
...
- (void)drawQuotaLinewithDataArr:(NSArray *)dataArr maxValue:(double)maxValue minValue:(double)minValue quotaName:(NSString *)quotaName subName:(NSString *)subName lineColor:(UIColor *)lineColor{
    ...
    if ([quotaName isEqualToString:MiddleQuotaName]&&NewQuotaHeight!=0) {
        ...
        if (maxValue!=minValue) {
            ...
            self.quotaVolHeightPerPoint = 0;
        }
        ZXQuotaLineLayer * MA6Layer = [[ZXQuotaLineLayer alloc] initQuotaDataArr:dataArr currentDrawStartIndex:self.needDrawStartIndex rowHeight:self.candleWidth minValue:self.quotaVolMinAssert heightPerpoint:self.quotaVolHeightPerPoint lineColor:lineColor quotaName:quotaName];
        //在同一个指标中，不同的指标线或者指标柱，数组中包含了同名的就提换，没有就新增
        NSMutableArray *tempArr = [NSMutableArray array];
        [self.volLayerArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (![self.quotaName isEqualToString:quotaName]) {
        if (self.quotaLayerArr.count>0&&![self.quotaName isEqualToString:MiddleQuotaName]) {
            for (NSDictionary *dic in self.quotaLayerArr) {
...
- (void)getLongPressDetailFromQuotaWithIndexInCurrentDrawKlineModelArr:(NSInteger)index
{
    ...
    NSAttributedString *quotaVolResultString = [self getQuotaResultStringWithDataArr:self.quotaVolDataArr colorArr:self.quotaVolColorArr index:index];
    if ([self.delegate respondsToSelector:@selector(shouldToReloadQuotaDetailViewWithResultString:shouldToReloadCandleDetailViewWithMAResultString:QuotaVolResultString:)]){
        
        [self.delegate shouldToReloadQuotaDetailViewWithResultString:quotaResultString shouldToReloadCandleDetailViewWithMAResultString:MAResultString QuotaVolResultString:quotaVolResultString];
    }
...
- (void)getCandyPriceWithPositionY:(CGFloat)positionY
{
    ...
    }else if (positionY>=self.candleChartHeight+TimeViewHeight+NewQuotaHeight+MiddleBlankSpace&&positionY<=self.subViewHeight)
    {
    ...
    }else if (positionY>=self.candleChartHeight+TimeViewHeight+MiddleBlankSpace&&positionY<=self.candleChartHeight+TimeViewHeight+NewQuotaHeight+MiddleBlankSpace)
    {
        //中间
        currentPositionPrice = (self.quotaVolMinAssert + ((self.subViewHeight-positionY-self.quotaChartHeight)/self.quotaVolHeightPerPoint));
        currentPositionPriceStr = [NSString stringWithFormat:@"%.0f",currentPositionPrice];
...
- (NSMutableArray *)volLayerArr
{
    if (!_volLayerArr) {
        _volLayerArr = [NSMutableArray array];
    }
    return _volLayerArr;
}
...
- (NSMutableArray *)quotaVolDataArr
{
    if (!_quotaVolDataArr) {
        _quotaVolDataArr = [NSMutableArray array];
    }
    return _quotaVolDataArr;
}
- (NSMutableArray *)quotaVolColorArr
{
    if (!_quotaVolDataArr) {
        _quotaVolDataArr = [NSMutableArray array];
    }
    return _quotaVolColorArr;
}

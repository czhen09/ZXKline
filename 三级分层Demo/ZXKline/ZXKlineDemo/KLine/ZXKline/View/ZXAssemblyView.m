 ...
 *  中部Vol指标细节显示view
 */
@property (nonatomic,strong) ZXCandleDetailView *quotaVolDetailView;
/**
 *  下部指标右侧坐标值显示
 */
 ...
 *  中部Vol指标右侧坐标值显示
 */
@property (nonatomic,strong) ZXPriceView *quotaVolView;
...
@property (nonatomic,strong) CAShapeLayer *middleBorder;
...
- (void)addSubviews
{
    ...
    if (!DrawJustKline) {
        ...
        [self addSubview:self.quotaVolDetailView];
        [self addSubview:self.quotaView];
        [self addSubview:self.quotaVolView];
...
- (void)initConstrainsWithWidth:(CGFloat)width height:(CGFloat)height
{
    ...
    if (!DrawJustKline) {
        [self.quotaView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.candleChartHeight+self.middleBlankSpace+TimeViewHeight+TopMargin+NewQuotaHeight);
        ...
        [self.quotaVolView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.candleChartHeight+self.middleBlankSpace+TimeViewHeight+TopMargin);
            make.left.mas_equalTo(self.priceView.mas_left);
            make.width.mas_equalTo(50);
            make.height.mas_equalTo(self.quotaChartHeight);
        }];
        [self.quotaVolView updateFrameWithHeight:(NewQuotaHeight)];
    }
    ...
    if (!DrawJustKline) {
        [self.quotaDetailView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(self.klineMainView).offset(self.candleChartHeight+self.middleBlankSpace+TimeViewHeight+NewQuotaHeight);
            if (PriceCoordinateIsInRight) {
        ...
        [self.quotaVolDetailView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)updateConstrainsWithWidth:(CGFloat)width height:(CGFloat)height
{
    ...
    if (!DrawJustKline) {
        ...
        [self.quotaView updateFrameWithHeight:self.quotaChartHeight];
        [self.quotaVolView mas_updateConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self.candleChartHeight+self.middleBlankSpace+TimeViewHeight+TopMargin);
            make.height.mas_equalTo(NewQuotaHeight);
        }];
        [self.quotaVolView updateFrameWithHeight:self.quotaChartHeight];
    }
    if (!DrawJustKline) {
        ...
        [self.quotaVolDetailView mas_updateConstraints:^(MASConstraintMaker *make) {
...
- (void)drawBorder
{
    ...
    if (!DrawJustKline) {
        ...
        if (PriceCoordinateIsInRight) {
            if (ZX_IS_IPHONE_X&&!Portrait) {
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(SafeAreaTopMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin+NewQuotaHeight,LanscapeCandleWidth, self.quotaChartHeight)];
            }else{
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(ZXLeftMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin+NewQuotaHeight,TotalWidth-ZXLeftMargin-ZXRightMargin-VerticalCoordinatesWidth, self.quotaChartHeight)];
            }
        }else{
            if (ZX_IS_IPHONE_X&&!Portrait) {
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(SafeAreaTopMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin+NewQuotaHeight,LanscapeCandleWidth, self.quotaChartHeight)];
            }else{
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(ZXLeftMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin+NewQuotaHeight,TotalWidth-ZXLeftMargin-ZXRightMargin, self.quotaChartHeight)];
            }
        }
    ...
    if (!DrawJustKline&&NewQuotaHeight!=0) {
        [self.middleBorder removeFromSuperlayer];
        self.middleBorder = nil;
        self.middleBorder = [CAShapeLayer layer];
        UIBezierPath *bottomBorderBeizer = nil;
        if (PriceCoordinateIsInRight) {
            if (ZX_IS_IPHONE_X&&!Portrait) {
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(SafeAreaTopMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin,LanscapeCandleWidth,NewQuotaHeight)];
            }else{
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(ZXLeftMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin,TotalWidth-ZXLeftMargin-ZXRightMargin-VerticalCoordinatesWidth, NewQuotaHeight)];
            }
        }else{
            if (ZX_IS_IPHONE_X&&!Portrait) {
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(SafeAreaTopMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin,LanscapeCandleWidth,NewQuotaHeight)];
            }else{
                bottomBorderBeizer = [UIBezierPath bezierPathWithRect:CGRectMake(ZXLeftMargin, self.candleChartHeight+TimeViewHeight+self.middleBlankSpace+TopMargin,TotalWidth-ZXLeftMargin-ZXRightMargin, NewQuotaHeight)];
            }
        }
        self.middleBorder.lineWidth = 0.2;
        self.middleBorder.path = bottomBorderBeizer.CGPath;
        self.middleBorder.strokeColor = GrateLineColor.CGColor;
        self.middleBorder.fillColor = [UIColor clearColor].CGColor;
        [self.layer addSublayer:self.middleBorder];
    }
...
//    }
    ...
    if (!DrawJustKline) {
        if ([quotaName isEqualToString:MiddleQuotaName]) {
            [self.quotaVolView reloadPriceLabelTextWithPriceArr:@[@(maxValue),@(minValue)] precision:0];
        }else{
...
- (void)shouldRenewChartDetailView
{
    ...
    if (!DrawJustKline) {
        ...
        [self.quotaVolDetailView reloadQuotaDetailViewWithQuotaDetailString:MiddleQuotaName];
    }
...
- (void)returnCurrentDrawKlineModelArr:(NSArray *)currentDrawKlineModelArr newKlineModel:(KlineModel *)newKlineModel
{
    ...
    if (!DrawJustKline) {
        if (NewQuotaHeight!=0) {
            [self drawPresetQuotaWithQuotaName:MiddleQuotaType currentDrawKlineModelArr:currentDrawKlineModelArr];
        }
...
- (void)shouldToReloadQuotaDetailViewWithResultString:(NSAttributedString *)QuotaResultString shouldToReloadCandleDetailViewWithMAResultString:(NSAttributedString *)MAResultString QuotaVolResultString:(NSAttributedString *)QuotaVolResultString
{
    if (!DrawJustKline) {
        ...
        //
        NSMutableAttributedString *attriQuotaVol = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@  ",MiddleQuotaName]];
        if (QuotaVolResultString) {
            [attriQuotaVol appendAttributedString:QuotaVolResultString];
        }
        [self.quotaVolDetailView reloadQuotaDetailViewWithQuotaAttributedString:attriQuotaVol];
    }
    if (IsDisplayStockOrQuotaName) {
...
{
    return self.candleChartHeight+QuotaChartHeight+MiddleBlankSpace+TimeViewHeight+NewQuotaHeight;
}
...
- (ZXCandleDetailView *)quotaVolDetailView
{
    if (!_quotaVolDetailView) {
        _quotaVolDetailView = [[ZXCandleDetailView alloc] init];
        _quotaVolDetailView.userInteractionEnabled = NO;
    }
    return _quotaVolDetailView;
}
- (ZXJumpView *)jumpView
{
    if (!_jumpView) {
...
- (ZXPriceView *)quotaVolView
{
    if (!_quotaVolView) {
        _quotaVolView = [[ZXPriceView alloc] initWithFrame:CGRectZero PriceArr:@[@"",@""]];
        _quotaVolView.userInteractionEnabled = NO;
    }
    return _quotaVolView;
}
